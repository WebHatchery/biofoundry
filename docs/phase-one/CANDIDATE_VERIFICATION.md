...
| Endless Worm Road Muster | Pass (focused, capture, and touch-audit evidence) | After the first Convoy, the repeatable data-driven Muster asks for four active Worm Outposts and sixteen post-Convoy scouting hauls, then pays 32 banked ingots per cleared network contract. The persisted claim count pays outstanding cycles exactly once, the completed-campaign Objective and route ledger expose live `4/4 routes · 15/16 hauls` and the reset next-cycle state, and the reward enters the same safe-beat notification path. [ui_endless_muster.png](../verification/ui_endless_muster.png) and [ui_endless_muster_awarded.png](../verification/ui_endless_muster_awarded.png) show the normal ledger states; [ui_compact_endless_muster.png](../verification/ui_compact_endless_muster.png) and [ui_compact_endless_muster_awarded.png](../verification/ui_compact_endless_muster_awarded.png) preserve the compact layout, while the official 800×450 touch audit reports no grown-target overlap. |
...
| Endless Worm Road Waypoint | Pass (focused, capture, and touch-audit evidence) | After a route has a Signal Cache and the Convoy has been cleared, it can spend 36 banked ingots once to install a Worm Road Waypoint. The route-specific transit helper shortens outbound and return worm trips from 18 to 12 seconds, while the inspection card, route ledger, Objective, notification, help copy, world-space route pulse, and midpoint Waypoint marker expose the purchase and `12s transit` payoff. The Waypoint flag persists, older saves default it safely off, load validation rejects bypassed Signal Cache or Convoy gates, and focused simulation/UI coverage pins the one-time spend. [ui_endless_waypoint.png](../verification/ui_endless_waypoint.png) and [ui_endless_waypoint_awarded.png](../verification/ui_endless_waypoint_awarded.png) show the purchase and success toast; [ui_endless_waypoint_in_flight.png](../verification/ui_endless_waypoint_in_flight.png) shows the corrected `12s remaining` transit state; compact variants keep the control, marker, and route summary visible, and the 800×450 touch audit reports no grown-target overlap. |
...
| Endless Wormsong Encore | Pass (focused, capture, and touch-audit evidence) | After Circuit, every three complete Wormsong Concord-boosted scouting hauls fill a repeatable data-driven Encore and award 20 banked ingots. The persisted haul and claim counters pay outstanding cycles exactly once, count only active routes carrying all four Wormsong roles, default safely for older saves, reject impossible predecessor or claimed-cycle combinations at the load boundary, and surface the next cycle through the selected Outpost card, Objective, route ledger, notification, and safe-beat autosave path. [ui_endless_wormsong_encore.png](../verification/ui_endless_wormsong_encore.png) shows the live `2/3 boosted hauls` counter across two complete routes, [ui_endless_wormsong_encore_awarded.png](../verification/ui_endless_wormsong_encore_awarded.png) shows the cleared reward and reset cycle, [ui_endless_wormsong_encore_inspect.png](../verification/ui_endless_wormsong_encore_inspect.png) keeps the compact `Archive 0/5 · Encore 2/3` line readable on the selected Outpost card, and the official 800×450 touch audit reports no grown-target overlap. |
...
| Endless automatic dispatch order | Pass (focused, capture, and touch-audit evidence) | The shared Worm's automatic route jobs now have a persisted global order: `Return first`, `Food first`, or `Load first`. The Routes ledger exposes `Cycle order`, announces the selected order, and names the exact next eligible route/job, while the fixed-step dispatcher uses the same order when multiple automatic policies are ready and retains the fair route cursor within each policy. Focused state and simulation coverage verify the cycle, preview, scheduler precedence, and legacy return-first default; [ui_endless_auto_priority.png](../verification/ui_endless_auto_priority.png) plus [ui_compact_endless_auto_priority.png](../verification/ui_compact_endless_auto_priority.png) show `Next automatic · Route 1 · outbound load` visible and tappable, and the 800×450 touch audit reports no overlap. |
...
| Backup-only load recovery | Pass (focused and published build evidence) | If a primary slot is missing while its conventional `_backup` survives, `Load` now validates the backup, restores the primary slot, and installs the recovered Warren; if the restore write is rejected, the valid backup remains playable, Continue stays available, and a persistent `RECOVERY FAILED · tap Save` banner keeps the repair action visible. The focused recovery-path coverage and published candidate include this branch, while ordinary Preview Continue remains verified separately. |
...
| Famine checkpoint preservation | Pass (focused and published Preview evidence) | Campaign autosaves now defer while the shared larder is empty, so a direct recovery decision cannot overwrite the last pre-crisis checkpoint. If the live Warren later becomes non-viable, `Load Last Safe` reads the validated backup and repairs the primary when storage permits; the published Preview recovery probe visibly reported `Safe checkpoint loaded and restored.`. Explicit manual Save remains available for players who intentionally want to checkpoint the current crisis state. |
...
| Endless Rest Hollow | Pass (focused and capture evidence) | Endless Build & Dig now exposes a passive Rest Hollow for 24 ore. Each finished Hollow adds the data-driven local workforce capacity without consuming a job slot, eases the existing crowding ratio, and reports `Providing room` plus its capacity payoff when inspected. The refreshed [ui_endless_rest_hollow.png](../verification/ui_endless_rest_hollow.png) capture keeps the new structure, build control, and inspection card visible. |
| Endless Wormsong Chorus | Pass (focused, capture, and touch-audit evidence) | After the Circuit, three active routes carrying complete Wormsong crews can claim the optional Chorus for 80 banked ingots. While all three complete routes remain intact, each route adds 1 ingot to every haul; the ledger, Objective, inspection card, notification, separate lifetime counter, legacy-save defaults, and recovery behavior expose when the chorus is live or paused. [ui_endless_wormsong_chorus.png](../verification/ui_endless_wormsong_chorus.png), [ui_endless_wormsong_chorus_awarded.png](../verification/ui_endless_wormsong_chorus_awarded.png), and [ui_endless_wormsong_chorus_haul.png](../verification/ui_endless_wormsong_chorus_haul.png) show the normal progress, reward, and live haul states; the matching [compact progress](../verification/ui_compact_endless_wormsong_chorus.png), [compact reward](../verification/ui_compact_endless_wormsong_chorus_awarded.png), and [compact haul](../verification/ui_compact_endless_wormsong_chorus_haul.png) captures keep the touch layout covered at 800×450, and the official audit sweep reports no grown-target overlap. |
...
| Study Pen adaptation payoff | Pass (focused and capture evidence) | The existing capture → study → adapt loop now has two real payoffs: 12 study unlocks `Adaptive Haulers`, increasing Beetle Hauler capacity by 20% (5 to 6), and 24 study unlocks `Brood Memory`, shortening Breeding Pit hatch cycles by 20% (150s to 120s). Multiple Study Pens stack their observation rate, while an empty pen reports `Waiting for specimens`; the Study Pen inspection shows specimens, study rate, observation total, and the current/next adaptation, and the Breeding Pit reports its live hatch cycle. Focused simulation, capacity, notice, and inspection coverage pin both thresholds, stacking, and legacy-save behavior. Refreshed [ui_study.png](../verification/ui_study.png) and [ui_compact_study.png](../verification/ui_compact_study.png) captures show the mastered adaptation readout; [ui_study_expansion.png](../verification/ui_study_expansion.png) and [ui_compact_study_expansion.png](../verification/ui_compact_study_expansion.png) show two pens doubling the observation rate, while [ui_study_empty.png](../verification/ui_study_empty.png) and [ui_compact_study_empty.png](../verification/ui_compact_study_empty.png) show the empty-pen recovery state at normal and compact sizes. |
...
| Compact viewport exploration | Pass with follow-up | The hosted Preview smoke path at 800×450 keeps the title, field guide, and Warren HUD on-canvas; capture probes at 800×450, 1024×576, 1280×720, and 1440×900 keep the Food/Factory/Worm tutorial cards, Blacksmith queue controls, Shrine pause control, completion choices, and Endless outpost actions visible without clipping or overlap. The compact branch now gives the high-frequency top-bar, Jobs, Build & Dig, Outpost, Blacksmith, Breeding Pit, and Shrine controls larger visual affordances; the Outpost card pairs return choices and sequential upgrades so Pause scouting and final Load remain visible, and the refreshed warning captures keep their messages clear of that row. Representative [ui_compact_warren.png](../verification/ui_compact_warren.png), [ui_compact_blacksmith.png](../verification/ui_compact_blacksmith.png), [ui_compact_tutorial_blacksmith.png](../verification/ui_compact_tutorial_blacksmith.png), [ui_compact_breeding.png](../verification/ui_compact_breeding.png), [ui_compact_shrine.png](../verification/ui_compact_shrine.png), [ui_compact_endless_load_preview.png](../verification/ui_compact_endless_load_preview.png), [ui_compact_endless_upgraded.png](../verification/ui_compact_endless_upgraded.png), and [ui_compact_endless_wormsong_chorus_haul.png](../verification/ui_compact_endless_wormsong_chorus_haul.png) captures preserve the responsive states. The compact Blacksmith now uses a two-column recipe card so the complete 13-recipe catalogue remains visible even below an open factory tutorial; concise recipe and gate labels remain readable, while the Breeding Pit retains 36-pixel action targets with 40-pixel spacing. Remaining text density and first-time-player comprehension still require human validation. |
| Tutorial direct-control wording | Pass (focused, capture, and test evidence) | The Food lesson now says `Tap Farm, then tap open floor`, and the Factory lesson names `Tap Blacksmith in Build & Dig, then tap open floor` before the visible Jobs reassignment and `tap Iron Pickaxe` action. Refreshed [ui_tutorial_food.png](../verification/ui_tutorial_food.png), [ui_tutorial_factory.png](../verification/ui_tutorial_factory.png), and [ui_compact_tutorial_blacksmith.png](../verification/ui_compact_tutorial_blacksmith.png) captures keep the explicit sequence rendered without clipping. |
| Objective direct-control wording | Pass (focused, capture, and test evidence) | Campaign handoffs and Endless recovery objectives now use explicit visible actions for Blacksmith, Mine, Worm Shrine, and the first awakened Outpost route. The focused [ui_victory_factory.png](../verification/ui_victory_factory.png) capture names `Continue to Factory`, Blacksmith in Build & Dig, and open-floor placement; [ui_endless_route_build.png](../verification/ui_endless_route_build.png) names the Outpost button, open-floor placement, and `Activate route`. Objective regression assertions cover the placement and activation handoffs. |
...
| Current WebGL onboarding smoke | Pass (developer evidence) | The locally published `352e307` candidate was opened in a real browser at 1280×720. Visible New Warren confirmation and fresh-run controls remained on-canvas; primary-pointer map dragging advanced Tutorial 1/5 to 2/5, Farm selection and open-floor placement created a 10-ore construction site, and the visible `− Miner` then `+ Carrier` recovery instruction reassigned the workforce while food pressure remained forecast. The run stopped during construction delivery, so this is not a qualifying first-time-player campaign result. |
...
- Optional compact probes — pass; temporary 800×450 captures kept specialist,
  ...
  captures keep the new Dispatch policy and departure notice visible.
   ...
   with 46-pixel recipe actions; [ui_compact_tutorial_blacksmith.png](../verification/ui_compact_tutorial_blacksmith.png)
   covers the same card while the factory tutorial remains open.
...
- Endless Outpost resonance beacon — pass; a route with the survey rig can
  purchase the save-compatible 24-ingot beacon once, shortening the
  data-driven scouting cycle from 30 to 20 seconds. The inspection hint and
  route ledger expose the cycle, the simulation uses the shorter value for
  both progress and completion, and the completed-campaign Objective counts
  the installed route upgrades. Focused simulation, persistence, Objective,
  and UI coverage passes; refreshed
...
- Endless Worm Road Archive — pass; after the one-time Charter, each five-haul
  page pays 8 banked ingots once, persists and displays the claimed-page count,
  and reports the reward through the safe-beat notification path. The first
  page also unlocks the stronger Archive Wayfinder carrier recipe. The route
  card, ledger, Objective, and player copy keep both the repeatable target and
  the accumulated page history visible. Refreshed
  [ui_endless_archive.png](../verification/ui_endless_archive.png) and
  [ui_endless_archive_awarded.png](../verification/ui_endless_archive_awarded.png)
  captures show the 4/5 setup and reward toast, while compact variants and the
  800×450 touch audit preserve the route controls without grown-target overlap.
- Endless Archive Wayfinder payoff — pass; the first claimed page adds a
  save-compatible 14-ingot carrier recipe with a +4 hauling bonus, exposes the
  exact `log 1 Archive page` gate before unlock, and lets auto-equip replace a
  weaker carrier tool after crafting. Focused data, notification, persistence,
  simulation, UI, and capture coverage passes; refreshed
...
- Endless Worm Road Muster — pass; after the first Convoy, four active
  Outposts and sixteen post-Convoy scouting hauls award a save-compatible
  32-ingot payout per repeatable network contract. The persisted claim count
  pays outstanding cycles exactly once, the Objective and route ledger expose
  the live 4/4 and 15/16 target plus the reset next-cycle state, and the
  milestone uses the safe-beat notification path. Focused data, simulation,
  state, Objective, notification, ledger, capture, and touch-audit coverage
  passes. Refreshed [ui_endless_muster.png](../verification/ui_endless_muster.png),
  [ui_endless_muster_awarded.png](../verification/ui_endless_muster_awarded.png),
  [ui_compact_endless_muster.png](../verification/ui_compact_endless_muster.png),
  and [ui_compact_endless_muster_awarded.png](../verification/ui_compact_endless_muster_awarded.png)
  keep both ledger layouts readable without grown-target overlap.
- Endless Worm Road Waypoint — pass; after Signal Cache and a cleared Convoy,
  an active route spends 36 banked ingots once to shorten Worm Road transit
  from 18 to 12 seconds. The save-compatible route flag, Objective guidance,
  route policy summary, inspection detail, success notification, load gates,
  legacy default, simulation timing, route pulse timing, and focused UI
  assertions all pass.
  Refreshed [ui_endless_waypoint.png](../verification/ui_endless_waypoint.png),
  [ui_endless_waypoint_awarded.png](../verification/ui_endless_waypoint_awarded.png),
  [ui_compact_endless_waypoint.png](../verification/ui_compact_endless_waypoint.png),
  and [ui_compact_endless_waypoint_awarded.png](../verification/ui_compact_endless_waypoint_awarded.png)
  show the purchase and payoff, while
  [ui_endless_waypoint_in_flight.png](../verification/ui_endless_waypoint_in_flight.png)
  shows the corrected 12-second transit state; the official 800×450 audit
  reports no grown-target overlap.
...
- Endless Worm Road Charter — pass; after three completed scouting hauls across
  ...
  exposes the live haul count and reward, the simulation emits a safe-beat
  success event, and focused simulation, save-compatibility, Objective, and
  capture coverage pass. Refreshed
...
- Endless Charter equipment payoff — pass; the one-time Charter now unlocks the
  save-compatible Wormbone toolkit: an 8-ingot Wormbone Drill that supersedes
  the Iron Pickaxe's 1.5× mining multiplier with 2×, an 8-ingot Wormbone
  Hauling Frame that improves a carrier's bonus capacity from +1 to +3, a
  10-ingot Wormbone Smith's Hammer that shortens craft time from 0.75× to 0.6×,
  and a 10-ingot Wormbone Guard Blade that doubles guard damage. The Blacksmith's
  disabled recipes name the exact `Charter required` gate before the milestone
  and become visible craft targets afterward; queueing funded orders moves banked
  ingots into the Blacksmith buffer, save validation rejects gated gear in an
  ...
  captures show the locked and funded unlocked states, with the 800×450 touch
  audit reporting no grown-target overlap.
- Endless Muster equipment payoff — pass; the first held Worm Road Muster now
  unlocks a save-compatible four-role Wormsong toolkit: a 22-ingot +6 carrier
  Harness, a 26-ingot 2.5× miner Drill, a 28-ingot 0.5× smith's Hammer, and a
  28-ingot 2.5× guard Blade. The Blacksmith shows the exact `hold 1 Worm Road
  Muster` gate, funded queueing uses the existing ingot buffer, and auto-equip
  replaces weaker Wormbone tools. Refreshed
  [ui_endless_muster_harness.png](../verification/ui_endless_muster_harness.png)
  and [ui_compact_endless_muster_harness.png](../verification/ui_compact_endless_muster_harness.png)
  captures show the recipe at both layout sizes and its concise compact labels;
  the official 800×450 touch
...
- Endless Wormsong Concord — pass; after a held Muster, one carrier, miner,
  smith, and guard kit stationed on active routes completes the one-time
  four-role network goal for 48 banked ingots. The progress helper counts
  distinct roles, ignores inactive routes, the reward enters the safe-beat
  notification/autosave path exactly once, and older saves default the claim
  to false. After the claim, a complete four-role crew on one active route
  keeps the song going with +1 ore per haul and -2s scouting cycle; incomplete
  or unclaimed routes remain neutral. Focused simulation, state, Objective,
...
- Endless Wormsong Encore — pass; after Circuit, complete Concord crews add one
  boosted haul to a persisted global counter each time they finish scouting.
  Every three boosted hauls pays 20 banked ingots repeatably, with outstanding
  cycles claimed exactly once and partial progress preserved after each reward.
  The save-compatible state defaults safely for older sessions, rejects
  impossible predecessor or claimed-cycle combinations before installation,
  the safe-beat notification/autosave path reports the payout, and the Objective, route
  ledger, selected Outpost card, field guide, capture scenes, and 800×450 touch
  audit keep the next cycle visible. [ui_endless_wormsong_encore.png](../verification/ui_endless_wormsong_encore.png),
  [ui_endless_wormsong_encore_awarded.png](../verification/ui_endless_wormsong_encore_awarded.png),
  and [ui_endless_wormsong_encore_inspect.png](../verification/ui_endless_wormsong_encore_inspect.png)
  show the live, cleared/reset, and compact inspection states.
...
- Endless automatic dispatch preview — pass; the route ledger derives the
  next automatic job from the selected global priority, the shared Worm's
  availability, and the same fair route cursor used by the fixed-step
  scheduler. It names `Next automatic · Route 1 · outbound load` when the
  competing scene has a ready load-first route and changes to `Automatic jobs
  · waiting for Worm` during transit, so the priority control has a visible
  consequence before the next tick. Focused preview and scheduler coverage
  pass, with normal and compact captures linked above.
...
- Active-run load confirmation — pass; the top-bar Load action now pauses the
  ...
  complete frame for world input, so a tap on its dimmed background cannot
  select or modify the Warren. Title Continue and non-viable
  recovery retain their direct Load paths, and both successful and failed
  loads clear the confirmation state before returning control to the Warren.
  Refreshed
  [ui_load_confirm.png](../verification/ui_load_confirm.png) and
  [ui_compact_load_confirm.png](../verification/ui_compact_load_confirm.png)
  captures verify the normal and compact modal layout; the resolved
  [ui_load_confirm_resolved.png](../verification/ui_load_confirm_resolved.png)
...
- Settled touch-target audit — pass; `scripts/audit_touch_targets.ps1` walks
  ...
  Outpost, automatic outbound loading enabled and in-flight, automatic dispatch
  ...
  logical pixels or more; the compact recipe card reports 46-pixel drawn
...
- Optional support capture — [ui_optional.png](../verification/ui_optional.png)
  ...
  the still-available `Slime · clean` and `Bat · 8 cargo` recruit controls now
  show their ongoing `+3 food/min` and `+4 food/min` draw on a second line.
  Available Beetle and Salamander actions receive the same disclosure before
  they are posted, with the latter naming its `1 charcoal/batch` meal. The
...
- Study scaling capture — [ui_study_expansion.png](../verification/ui_study_expansion.png)
  shows two Study Pens raising the inspection rate to `+4.0/min`.
- Empty Study Pen capture — [ui_study_empty.png](../verification/ui_study_empty.png)
  shows the truthful `Waiting for specimens` status and the first `gain 12 study`
  recovery target.
...
- Secured-warren overlay handoff — once the Guard is assigned, the
  `Warren Secured` report now names the visible `Continue to Factory` button,
  the `Blacksmith` Build & Dig control, and open-floor placement before the
  ingot goal. The compact [ui_victory_factory.png](../verification/ui_victory_factory.png)
  capture and HUD regression assertions keep that transition explicit.
...
- Persistent save-failure recovery — pass; a rejected save keeps a compact
  `SAVE FAILED · checkpoint safe` or `SAVE FAILED · tap Save` banner beside the
  visible Save retry control until a successful save or load clears it. The full
  storage error remains in the transient notification and Recent Events, while
  the deterministic [ui_save_failure.png](../verification/ui_save_failure.png)
  and [ui_save_failure_first_save.png](../verification/ui_save_failure_first_save.png)
  captures verify both checkpoint states. If a validated backup cannot be
  written back to the primary slot, Continue remains enabled and the separate
  `RECOVERY FAILED · tap Save` banner keeps repair visible in
  [ui_save_recovery_failure.png](../verification/ui_save_recovery_failure.png).
  These captures prove the recovery UI, not forced browser quota behavior; that
  live-storage check remains open below.
...
- Factory Objective guidance — when the Blacksmith exists but no Smith is
  ...
  handoff.
- Awakened Outpost handoff — the first Endless route objective now names the
  visible `Outpost` Build & Dig button, open-floor placement, and `Activate
  route`; an already placed but inactive route also names the inspection and
  activation taps. The compact [ui_endless_route_build.png](../verification/ui_endless_route_build.png)
  capture keeps the first build instruction and controls readable together.
...
- Multi-route ledger capture — [ui_endless_routes.png](../verification/ui_endless_routes.png)
  ...
  live Worm Road Charter progress/reward, and attention states—including a
  crewless active route—before a card is opened. A compact 800×450 probe kept
  the same controls on-canvas without overlap.
- Shared-worm transit capture — [ui_endless_routes_busy.png](../verification/ui_endless_routes_busy.png)
  and [ui_compact_endless_routes_busy.png](../verification/ui_compact_endless_routes_busy.png)
  add the ledger's explicit `Worm in transit · Route 1 returning · 7s remaining`
  diagnostic above the same two inspectable route cards; the waiting card now
  reads `Worm busy · payload ready` instead of implying immediate dispatch. The
  settled 800×450 touch audit reports two Inspect targets and Close with no
  grown-target overlap.
