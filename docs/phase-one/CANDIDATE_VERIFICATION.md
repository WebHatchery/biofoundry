...
| Shared-worm transit diagnostic | Pass (capture and touch-audit evidence) | While one route is traveling, the Worm Route Ledger now adds a warning line naming the route, direction, and seconds remaining: `Worm in transit · Route 1 returning · 7s remaining`. The other route cards remain inspectable, so the single-worm bottleneck is visible without implying that a ready route has stalled. Refreshed [ui_endless_routes_busy.png](../verification/ui_endless_routes_busy.png) and [ui_compact_endless_routes_busy.png](../verification/ui_compact_endless_routes_busy.png) show the normal and 800×450 states; the settled compact touch audit reports no grown-target overlap. |
...
- Shared-worm transit capture — [ui_endless_routes_busy.png](../verification/ui_endless_routes_busy.png)
  ...
  diagnostic above the same two inspectable route cards. The settled 800×450
  touch audit reports two Inspect targets and Close with no grown-target overlap.
