...
| Endless Worm Road Waypoint | Pass (focused, capture, and touch-audit evidence) | After a route has a Signal Cache and the Convoy has been cleared, it can spend 36 banked ingots once to install a Worm Road Waypoint. The route-specific transit helper shortens outbound and return worm trips from 18 to 12 seconds, while the inspection card, route ledger, Objective, notification, and help copy expose the purchase and `12s transit` payoff. The Waypoint flag persists, older saves default it safely off, load validation rejects bypassed Signal Cache or Convoy gates, and focused simulation/UI coverage pins the one-time spend. [ui_endless_waypoint.png](../verification/ui_endless_waypoint.png) and [ui_endless_waypoint_awarded.png](../verification/ui_endless_waypoint_awarded.png) show the purchase and success toast; compact variants keep the control and route summary visible, and the 800×450 touch audit reports no grown-target overlap. |
...
- `cargo test --all-targets` — 414 unit tests and 2 integration/code-standard
  targets pass,
...
- Endless Worm Road Waypoint — pass; after Signal Cache and a cleared Convoy,
  ...
  legacy default, simulation timing, and focused UI assertions all pass.
  ...
  show the purchase and payoff; the official 800×450 audit reports no grown-
  target overlap.
