...
| Objective direct-control wording | Pass (focused, capture, and test evidence) | Campaign and Endless recovery objectives now use explicit visible actions for Blacksmith, Mine, Worm Shrine, and the first awakened Outpost route. The focused [ui_endless_route_build.png](../verification/ui_endless_route_build.png) capture names `Outpost` in Build & Dig, open-floor placement, and `Activate route`; objective regression assertions cover the placement and activation handoffs. |
