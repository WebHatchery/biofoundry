...
| Objective direct-control wording | Pass (focused, capture, and test evidence) | Campaign and Endless recovery objectives now use the same explicit `tap open floor` placement language for Blacksmith, Mine, and Worm Shrine steps. The refreshed [ui_factory_complete.png](../verification/ui_factory_complete.png) capture shows the updated Shrine instruction, and objective regression assertions cover the Blacksmith and Mine guidance. |
