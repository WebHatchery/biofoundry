...
| Direct player decision persistence | Pass (focused and published build evidence) | Successful job reassignment, specialist recruitment, breeding, Blacksmith queueing, tutorial skip, and milestone-report dismissal now write an immediate autosave. Failed or duplicate actions do not write, and the refreshed save roundtrip covers the persisted job, queue, and decision flags. |
...
- Direct player decision persistence — pass; successful job reassignment,
  specialist recruitment, breeding, Blacksmith queueing, tutorial skip, and
  milestone-report dismissal now trigger immediate autosaves. Failed or
  duplicate actions do not write, and save-roundtrip coverage preserves the
  resulting job, queue, and decision flags. The published Preview build
  includes the updated action path.
