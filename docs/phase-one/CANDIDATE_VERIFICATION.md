...
| Endless Worm Road Archive | Pass (focused, capture, and touch-audit evidence) | After the one-time Charter, every five additional scouting hauls fill a repeatable Worm Road Archive page for 8 banked ingots. The persisted claim count pays all outstanding pages exactly once, the simulation reports the reward as a safe-beat event, and the completed-campaign Objective plus route ledger keep the next page visible. [ui_endless_archive.png](../verification/ui_endless_archive.png) shows `Archive 4/5` and the next-action guidance; [ui_endless_archive_awarded.png](../verification/ui_endless_archive_awarded.png) shows the haul and `+8 ingots · 1 page logged` notification. Compact variants preserve the route controls, and the 800×450 touch audit reports no grown-target overlap. |
...
- `cargo test --all-targets` — 373 unit tests and 2 integration/code-standard
  targets pass,
...
- Endless Worm Road Archive — pass; after the one-time Charter, each five-haul
  page pays 8 banked ingots once, persists the claimed-page count, and reports
  the reward through the safe-beat notification path. The route card, ledger,
  Objective, and player copy keep the repeatable target visible. Refreshed
