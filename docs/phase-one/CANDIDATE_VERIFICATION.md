...
| Compact viewport exploration | Pass with follow-up | The current 800×450 capture sweep keeps the title, settings, Warren HUD, recovery cards, Field Guide, event log, Shrine pause action, completion choices, and route ledger on-canvas without grown-target overlap. The compact branch now gives the high-frequency top-bar, Jobs, Build & Dig, tutorial, recovery, guide, event-log, route-ledger, completion, and Shrine actions 72px logical touch-sized controls, reflows the narrow confirmation/settings cards, and keeps the core panels inside the viewport. Optional specialist and detailed Outpost inspection controls remain dense and are still a human readability follow-up. |
...
- `cargo test --all-targets` — 511 unit tests and 2 integration/code-standard
  targets pass,
...
- Optional compact probes — pass; temporary 800×450 captures kept specialist,
  ...
  captures keep the new Dispatch policy and departure notice visible. The
  focused compact touch-audit sweep also reports no grown-target overlap for
  the core shell, while advisory sizing still flags the dense optional
  specialist and Outpost inspection controls for a future pass.
