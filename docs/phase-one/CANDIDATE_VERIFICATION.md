...
| Endless automatic dispatch order | Pass (focused, capture, and touch-audit evidence) | The shared Worm's automatic route jobs now have a persisted global order: `Return first`, `Food first`, or `Load first`. The Routes ledger exposes `Cycle order`, announces the selected order, and the fixed-step dispatcher uses it when multiple automatic policies are ready while retaining the fair route cursor within each policy. Focused state and simulation coverage verify the cycle, legacy saves default to return-first, and [ui_endless_auto_priority.png](../verification/ui_endless_auto_priority.png) plus [ui_compact_endless_auto_priority.png](../verification/ui_compact_endless_auto_priority.png) show the choice visible and tappable; the 800×450 touch audit reports no overlap. |
