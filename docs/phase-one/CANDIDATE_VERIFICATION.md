...
- `cargo test --all-targets` — 359 unit tests and 2 integration/code-standard
  targets pass,
...
- Endless Charter equipment payoff — pass; the one-time Charter now unlocks the
  ...
  visible craft target afterward; queueing a funded order moves banked ingots into
  the Blacksmith buffer, save validation rejects gated gear in an unclaimed
  session, and the auto-equip loop chooses the strongest unlocked matching tool.
  Focused data, game-state, persistence, and simulation coverage passes; refreshed
  [ui_compact_blacksmith.png](../verification/ui_compact_blacksmith.png) and
  [ui_compact_endless_wormbone_drill.png](../verification/ui_compact_endless_wormbone_drill.png)
