...
| Current WebGL onboarding smoke | Pass (developer evidence) | The locally published candidate was opened in a real browser at 1280×720. Visible New Warren confirmation and fresh-run controls remained on-canvas; primary-pointer map dragging advanced Tutorial 1/5 to 2/5, Farm selection and open-floor placement created a 10-ore construction site, and the visible `− Miner` then `+ Carrier` recovery instruction reassigned the workforce while food pressure remained forecast. The run stopped during construction delivery, so this is not a qualifying first-time-player campaign result. |
