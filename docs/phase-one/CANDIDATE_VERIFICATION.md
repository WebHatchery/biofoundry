...
| Endless Charter equipment payoff | Pass (focused and capture evidence) | Claiming the Worm Road Charter unlocks the data-driven Wormbone toolkit: the 8-ingot Wormbone Drill for miners and the 8-ingot Wormbone Hauling Frame for carriers. The Blacksmith keeps both recipes disabled with an explicit `Charter required` label until the milestone is claimed; afterward visible recipes can be queued, and any banked Charter ingots are reserved into the Blacksmith buffer at queue time so funded orders can start immediately. The auto-equip loop prefers each Wormbone item over its weaker baseline counterpart, improving mining speed and carrier capacity. Focused gate, persistence, simulation, and content-validation coverage pass; refreshed [ui_compact_blacksmith.png](../verification/ui_compact_blacksmith.png) shows both locked recipes, while [ui_compact_endless_wormbone_drill.png](../verification/ui_compact_endless_wormbone_drill.png) shows the funded unlocked toolkit and the 800×450 touch audit reports no grown-target overlap. |
...
- `cargo test --all-targets` — 360 unit tests and 2 integration/code-standard
  targets pass,
...
- Endless Charter equipment payoff — pass; the one-time Charter now unlocks the
  ...
  the Iron Pickaxe's 1.5× mining multiplier with 2×, plus an 8-ingot Wormbone
  Hauling Frame that improves a carrier's bonus capacity from +1 to +3. The
  Blacksmith's disabled recipes name the exact `Charter required` gate before
  the milestone and become visible craft targets afterward; queueing funded
  orders moves banked ingots into the Blacksmith buffer, save validation rejects
  gated gear in an unclaimed session, and the auto-equip loop chooses the
  strongest unlocked matching tool. Focused data, game-state, persistence, and
  simulation coverage passes; refreshed [ui_compact_blacksmith.png](../verification/ui_compact_blacksmith.png)
