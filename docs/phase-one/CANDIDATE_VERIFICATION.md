...
| Study Pen adaptation payoff | Pass (focused and capture evidence) | The existing capture → study → adapt loop now has a real payoff: a Study Pen converts housed specimens into observation, and 12 study unlocks `Adaptive Haulers`, increasing Beetle Hauler capacity by 20% (5 to 6). The inspection card shows specimens, study rate, observation total, and either live `gain 12 study` progress or `Adapted haulers · +20% carry`; focused simulation, capacity, notice, and inspection coverage pin the threshold and legacy-save behavior. Refreshed [ui_study.png](../verification/ui_study.png) and [ui_compact_study.png](../verification/ui_compact_study.png) captures show the completed adaptation. |
...
- Study adaptation capture — [ui_study.png](../verification/ui_study.png) and
  ...
  Pen with two housed specimens, `+2.0/min` observation, `12.0 observed`, and the
  unlocked `Adapted haulers · +20% carry` payoff at both viewport sizes.
