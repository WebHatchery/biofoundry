...
| Study Pen adaptation payoff | Pass (focused and capture evidence) | The existing capture → study → adapt loop now has two real payoffs: 12 study unlocks `Adaptive Haulers`, increasing Beetle Hauler capacity by 20% (5 to 6), and 24 study unlocks `Brood Memory`, shortening Breeding Pit hatch cycles by 20% (150s to 120s). The Study Pen inspection shows specimens, study rate, observation total, and the current/next adaptation; the Breeding Pit reports its live hatch cycle. Focused simulation, capacity, notice, and inspection coverage pin both thresholds and legacy-save behavior. Refreshed [ui_study.png](../verification/ui_study.png) and [ui_compact_study.png](../verification/ui_compact_study.png) captures show the mastered adaptation readout. |
