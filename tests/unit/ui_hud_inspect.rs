...
use biofoundry::state::creatures::{Good, Job};
use biofoundry::state::outposts::CargoPriority;
...
use biofoundry::ui::hud::inspect::breeding::breed_label;
use biofoundry::ui::hud::inspect::*;
use macroquad::prelude::*;
use macroquad_toolkit::grid::TilePos;
use macroquad_toolkit::prelude::dark;

#[path = "ui_hud_inspect/encore.rs"]
mod encore;
#[path = "ui_hud_inspect/equipment.rs"]
mod equipment;
#[path = "ui_hud_inspect/outpost_details.rs"]
mod outpost_details;
#[path = "ui_hud_inspect/route_status.rs"]
mod route_status;
...
fn unreachable_workstation_inspection_names_the_tunnel_recovery_status() {
    ...
    let pos = session
        ...
        .iter_with_pos()
        .find(|(pos, tile)| {
            **tile == biofoundry::state::world::Tile::Floor
                && *pos != stockpile
    ...
    for neighbor in pos.neighbors_4way() {
        session
            .world
            .tiles
            .set(neighbor, biofoundry::state::world::Tile::Rock);
    }
...
fn in_flight_outpost_reports_directional_transit() {
    ...
    assert!(biofoundry::simulation::outposts::start_to_outpost(
        &mut session,
        &data,
        pos
    ));
...
fn empty_outpost_does_not_count_remote_crew_as_ready_to_load() {
    ...
    session.economy.food = data.balance.worm_feed_reserve;
    session.creatures.clear();
    session.spawn_creature(&data, "goblin", biofoundry::state::creatures::Job::Carrier);
...
fn claimed_outpost_reports_archive_history_and_next_page() {
    ...
    let mut outpost = biofoundry::state::outposts::Outpost::new(TilePos::new(4, 4));
...
fn outpost_load_hint_previews_the_selected_priority_and_reserve() {
    ...
    let mut outpost = biofoundry::state::outposts::Outpost::new(pos);
