...
use biofoundry::game::persistence::validate_loaded_session;
use biofoundry::game::*;
...
#[path = "game/persistence.rs"]
mod persistence;
#[path = "game/safety.rs"]
mod safety;
#[path = "game/unlocks.rs"]
mod unlocks;
...
fn current_version_save_payloads_still_receive_integrity_validation() {
    ...
    let error = biofoundry::game::persistence::validate_loaded_session_boundary(restored, &data)
...
fn notification_history_rehydrates_without_replaying_old_toasts() {
    ...
    biofoundry::game::persistence::restore_notification_history(&mut manager, &history);
...
fn loaded_session_validation_accepts_a_simulated_warren() {
    let (data, mut session) = session();
    for _ in 0..600 {
        biofoundry::simulation::tick(&mut session, &data);
    }
...
fn loaded_session_validation_accepts_remote_transit_states() {
    ...
    biofoundry::simulation::outposts::tick_transit(
        &mut session,
        &data,
...
fn loaded_session_validation_rejects_remote_cargo_sum_overflow() {
    ...
    session.outposts[0]
        .cargo
        .insert(biofoundry::state::creatures::Good::Ore, u32::MAX);
    session.outposts[0]
        .cargo
        .insert(biofoundry::state::creatures::Good::Ingot, 1);
...
fn loaded_session_validation_rejects_unsupported_remote_cargo() {
    ...
    session.outposts[0]
        .cargo
        .insert(biofoundry::state::creatures::Good::Wood, 1);
