...
#[test]
fn player_copy_loads_as_typed_required_messages() {
    let data = GameData::load().expect("embedded game data");

    assert_eq!(data.copy.len(), REQUIRED_COPY_IDS.len());
    for id in REQUIRED_COPY_IDS {
        assert!(!data.message(id).trim().is_empty(), "empty copy: {id}");
    }
    assert!(data.message("menu.tagline").contains("conveyor"));
    assert!(data.message("load.confirmation").contains("discarded"));
}
...
fn balance_values_are_playable() {
    ...
    assert!(b.rest_hollow_capacity > 0);
    ...
    assert!(b.outpost_chorus_route_goal >= 3);
    assert!(b.outpost_chorus_reward_ingots > b.outpost_circuit_reward_ingots);
    assert!(b.outpost_chorus_ingots_per_haul > 0);
