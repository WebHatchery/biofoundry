...
use biofoundry::state::outposts::{AutoRoutePriority, WormTransit};
use biofoundry::state::structures::Building;
use biofoundry::state::world::Tile;
use biofoundry::state::*;
...
#[test]
fn saves_from_before_muster_contract_load_with_no_contracts_claimed() {
    let data = GameData::load().unwrap();
    let session = GameSession::new(&data, 42);
    let mut encoded = serde_json::to_value(&session).unwrap();
    encoded
...
#[test]
fn saves_from_before_wormsong_encore_load_with_no_progress() {
    let data = GameData::load().unwrap();
    let session = GameSession::new(&data, 42);
    let mut encoded = serde_json::to_value(&session).unwrap();
    let object = encoded.as_object_mut().unwrap();
    object.remove("outpost_concord_hauls");
    object.remove("outpost_encore_claims");

    let restored: GameSession = serde_json::from_value(encoded).unwrap();

    assert_eq!(restored.outpost_concord_hauls, 0);
    assert_eq!(restored.outpost_encore_claims, 0);
}

#[test]
fn saves_from_before_wormsong_chorus_load_unclaimed() {
    let data = GameData::load().unwrap();
    let session = GameSession::new(&data, 42);
    let mut encoded = serde_json::to_value(&session).unwrap();
    encoded
        .as_object_mut()
        .unwrap()
        .remove("outpost_chorus_claimed");

    let restored: GameSession = serde_json::from_value(encoded).unwrap();

    assert!(!restored.outpost_chorus_claimed);
}
