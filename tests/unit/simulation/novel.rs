...
use super::boot;
use biofoundry::simulation::{self, outposts, SIM_DT};
use biofoundry::state::creatures::{Good, Job, Task};
use biofoundry::state::outposts::{CargoPriority, TransitDirection};
use biofoundry::state::structures::Building;
use macroquad_toolkit::grid::TilePos;

#[path = "novel/auto_load.rs"]
mod auto_load;
#[path = "novel/auto_priority.rs"]
mod auto_priority;
#[path = "novel/capacity.rs"]
mod capacity;
#[path = "novel/cargo_return.rs"]
mod cargo_return;
#[path = "novel/chorus.rs"]
mod chorus;
#[path = "novel/engineering.rs"]
mod engineering;
#[path = "novel/remote_specialists.rs"]
mod remote_specialists;
#[path = "novel/study.rs"]
mod study;
#[path = "novel/transit_failures.rs"]
mod transit_failures;

pub(super) fn active_outpost(
    seed: u64,
) -> (
    biofoundry::data::GameData,
    biofoundry::state::GameSession,
...
fn raw_food_spoils_into_waste_and_cooked_alias_persists() {
    ...
    let restored: biofoundry::state::GameSession = serde_json::from_str(&encoded).unwrap();
...
fn raw_food_ledger_matches_stock_after_spoilage() {
    ...
    biofoundry::simulation::colony::tick_spoilage(&mut session, &data, SIM_DT);
...
fn staffed_outpost_scouting_consumes_food_and_stores_remote_ore() {
    ...
    assert_eq!(
        completions,
        vec![biofoundry::state::outposts::ExpeditionCompletion {
...
fn in_flight_worm_transit_survives_a_save_roundtrip() {
    ...
    let restored: biofoundry::state::GameSession = serde_json::from_str(&json).unwrap();
