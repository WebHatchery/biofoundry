...
use biofoundry::ui::hud::panels::specialists::{optional_specialist_label, optional_support_label};
use biofoundry::ui::hud::panels::*;
use biofoundry::ui::legibility::advanced_systems_unlocked;
...
fn secure_tutorial_names_the_available_guard_action() {
    ...
    let step = biofoundry::tutorial::current_step(&session, &data).expect("secure tutorial step");
...
fn food_tutorial_names_the_available_carrier_action() {
    ...
    let step = biofoundry::tutorial::current_step(&session, &data).expect("food tutorial step");
...
fn tutorial_placement_and_crafting_steps_name_each_visible_tap() {
    ...
    session.tutorial_step = 1;
    let food = biofoundry::tutorial::current_step(&session, &data).expect("food tutorial step");
    ...
    let factory =
        biofoundry::tutorial::current_step(&session, &data).expect("factory tutorial step");
...
fn factory_tutorial_names_the_available_smith_action() {
    ...
    let mut session = GameSession::new(&data, 7);
    session.tutorial_step = 2;
    let step = biofoundry::tutorial::current_step(&session, &data).expect("factory tutorial step");
