...
const INSPECT_HELP_BODY: &str =
    "Tap a building on the map to see its status and controls. Tap an equipment button in the Blacksmith card to queue it; after onboarding, Breeding Pit buttons show specialist benefits. Tap a Study Pen to turn captured specimens into observation and unlock stronger Beetle Haulers. At an awakened Outpost, tap Load order for cargo priority, Crew per run for cargo-only or scout counts, then Load to send food and crew to scout for ore; pause scouting to protect provisions, use the cargo-only return to bring ore home while keeping scouts remote, turn on Auto-load · Cargo + crew to launch ready outbound runs, turn on Auto-return · Cargo only for full holds, turn on Auto-resupply · Food only when remote scouts need provisions, and tap Routes · Cycle order to choose which ready automatic job gets the shared Worm first. After that, expand the hold and camp with banked ingots, install a survey rig to increase ore per scout, then tune its resonance beacon to shorten survey cycles and keep scouting to log repeatable Archive pages for ingots. The first page unlocks the stronger Archive Wayfinder carrier recipe at the Blacksmith; after that, use Routes to run the Worm Road Relay across two active Outposts, run the repeatable Worm Road Convoy across three active routes, install a Signal Cache for ingots on later hauls, then add a Worm Road Waypoint to shorten transit.";
const OBJECTIVE_HELP_BODY: &str =
    "Read the Objective card for the current milestone. Locked gates name their exact unlock; after the worm wakes, tap Routes for every Outpost. The card tracks Charter hauls, claimed Archive pages, the Archive Wayfinder payoff from the first page, the Worm Road Relay, repeatable Worm Road Convoy contracts, the Signal Cache payoff, the Worm Road Waypoint transit payoff, the shared-Worm automatic dispatch order, and the next route target.";
