...
const INSPECT_HELP_BODY: &str =
    "Tap a building on the map to see its status and controls. Tap an equipment button in the Blacksmith card to queue it; after onboarding, Breeding Pit buttons show specialist benefits. At an awakened Outpost, tap Load order for cargo priority, Crew per run for cargo-only or scout counts, then Load to send food and crew to scout for ore; pause scouting to protect provisions before returning the hold.";
